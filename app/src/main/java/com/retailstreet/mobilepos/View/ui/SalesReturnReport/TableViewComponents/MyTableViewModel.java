...
public class MyTableViewModel {
    ...
    private List<ColumnHeaderModel> createColumnHeaderModelList() {
        ...
        list.add(new ColumnHeaderModel("TRANSACTION ID"));
        list.add(new ColumnHeaderModel("RETURN DATE"));
        list.add(new ColumnHeaderModel("RETURN MODE"));
    ...
    private List<List<CellModel>> createCellModelList(List<User> userList) {
        ...
        for (int i = 0; i < userList.size(); i++) {
            ...
            list.add(new CellModel(user.TRANS_ID , user.CASHIER,user.MASTER_ID));          // "Id"
            list.add(new CellModel(user.TRANS_ID , user.TRANS_ID,user.MASTER_ID));        // "Name"
            list.add(new CellModel(user.TRANS_ID , user.RETURN_DATE,user.MASTER_ID));    // "Nickname"
            list.add(new CellModel(user.TRANS_ID , user.RETURN_MODE,user.MASTER_ID));       // "Email"
            list.add(new CellModel(user.TRANS_ID , user.TOTAL,user.MASTER_ID));   // "BirthDay"// "Gender"
    ...
    private List<RowHeaderModel> createRowHeaderList(int size, List<User> users) {
        ...
        for (User user:users) {
            ...
            list.add(new RowHeaderModel(String.valueOf(i + 1),user.MASTER_ID,user.MASTER_ID));
            i++;
        }
        return list;
    }
