...
public class MyTableViewModel {
    ...
    private List<ColumnHeaderModel> createColumnHeaderModelList() {
        ...
        list.add(new ColumnHeaderModel("BILL NO"));
    ...
    private List<List<CellModel>> createCellModelList(List<User> userList) {
        ...
        for (int i = 0; i < userList.size(); i++) {
            ...
            list.add(new CellModel(user.BILL_NO , user.CASHIER,user.MASTER_ID));          // "Id"
            list.add(new CellModel(user.BILL_NO , user.BILL_NO,user.MASTER_ID));        // "Name"
    ...
    private List<RowHeaderModel> createRowHeaderList(int size, List<User> users) {
        ...
        for (User user:users) {
            ...
            list.add(new RowHeaderModel(String.valueOf(i + 1),user.BILL_NO,user.MASTER_ID));
            i++;
        }
        return list;
    }
